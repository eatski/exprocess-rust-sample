...
impl Component for Room {
    ...
    fn update(&mut self, ctx: &Context<Self>, msg: Self::Message) -> bool {
        match msg {
            Msg::UpdateRoom(room) => self.state = State::Fetched(room,get_your_id(ctx.props().room_id.as_str())),
            Msg::RoomNotExists => self.state = State::NotExists,
            Msg::Start => {
                self.state = State::Loading;
                let on_error = ctx.props().on_error.clone();
                start_room(&ctx.props().room_id,move || on_error.clone().emit(()));
            }
        };
        true
    }
        ...
        match &self.state {
            ...
            State::Fetched(room,your_id) => match room.phase {
                Phase::Meeting =>  if !room.is_host {
                    html! {<Meeting room_id={ctx.props().room_id.clone()} on_error={ctx.props().on_error.clone()}/>}
                } else {
                    let start = ctx.link().callback(|_| Msg::Start);
                    html! {<MeetingHost room_id={ctx.props().room_id.clone()} start={start} on_error={ctx.props().on_error.clone()}/>}
                },
                Phase::Started => match your_id {
                    Some(your_id) => html! {
                        <Main 
                            ...
                            on_error={ctx.props().on_error.clone()}
                        />
                    },
                    None => not_found(),
                }
